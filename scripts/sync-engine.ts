...
const PACKAGE_INTEGRITY =
  "sha512-Zgjx2xIf2RJf1gjvHGR8lvcLRfw54Cq48QFMOrOxtt3SeAf+/h58IbVNoYpZbK7O5O23GCwLeNqS4T6zODHpWA==";
...
try {
  ...
  if (!packageBytes) {
    if (offline)
      throw new Error(
        `Offline engine package cache is missing or invalid: ${packagePath}`,
      );
    emit("download", "start", {
    ...
    if (!response.ok)
      throw new Error(
        `Engine package download failed: HTTP ${response.status}`,
      );
    const contentLength = Number(response.headers.get("content-length") ?? 0);
    if (contentLength > MAX_PACKAGE_BYTES)
      throw new Error("Engine package exceeds size limit");
  ...
  if (
    ...
    throw new Error(
      "Engine package identity does not match the pinned package",
    );
  }
  const manifest = {
    schemaVersion: 1,
...
async function readValidCachedPackage(
  filePath: string,
...
function validatePackage(bytes: Uint8Array): void {
  ...
  if (actual !== PACKAGE_INTEGRITY)
    throw new Error("Engine package integrity mismatch");
}
